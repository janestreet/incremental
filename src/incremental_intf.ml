...
module type S = sig
  ...
  *)
  ...
  (** [memoize_fun f hashable] returns a function [m] that is a memoized version of [f]
      ...
      The weak versions of the functions use a {!Weak_hashtbl} for the memo table.  This
      keeps a weak pointer to each result, and so the garbage collector automatically
      removes unused results.  Furthermore, [stabilize] removes the table entries whose
      result is unused. *)
  val memoize_fun
...
module type Timing_wheel_config = sig
  val start : Time.t
  val config : Timing_wheel.Config.t
end
...
module type Incremental = sig
  ...
  (** [Make] returns a new incremental implementation.  Incremental uses a timing wheel
      for its time-based functions.  [Make] uses a default timing-wheel configuration,
      with one millisecond precision with alarms up to 30 days in the future.

      [Make_with_timing_wheel_config] allows one to specify the timing-wheel
      configuration. *)

  module Make                          ()             : S
  module Make_with_timing_wheel_config (C : Timing_wheel_config) () : S
