...
module As_recompute_list =
  Node.Packed.As_list (struct
    let next (node : _ Node.t) = node.next_in_recompute_heap
  end)

module Nodes_by_height = struct
  type t = As_recompute_list.t Array.t with sexp_of
...
let invariant t =
  Invariant.invariant _here_ t <:sexp_of< t >> (fun () ->
    ...
    Fields.iter
      ...
      ~nodes_by_height:(check (fun nodes_by_height ->
        Array.iteri nodes_by_height ~f:(fun height node ->
          As_recompute_list.iter node ~f:(fun node ->
...
let set_max_height_allowed t max_height_allowed =
  if verbose
  then Debug.ams _here_ "set_max_height_allowed" (max_height_allowed, t)
         <:sexp_of< int * t >>;
  if debug then
  ...
  Array.blit ~src ~src_pos:0 ~dst ~dst_pos:0
...
let link (type a) t (node : a Node.t) =
  ...
  let next = Array.get t.nodes_by_height height in
  node.next_in_recompute_heap <- next;
  if Uopt.is_some next
  then (Uopt.unsafe_value next).prev_in_recompute_heap <- Uopt.some (Node.pack node);
...
let unlink (type a) t (node : a Node.t) =
  ...
  if phys_same
       (Uopt.some node)
       (Array.get t.nodes_by_height node.height_in_recompute_heap)
  then Array.unsafe_set t.nodes_by_height node.height_in_recompute_heap next;
  if Uopt.is_some next then (Uopt.unsafe_value next).prev_in_recompute_heap <- prev;
  if Uopt.is_some prev then (Uopt.unsafe_value prev).next_in_recompute_heap <- next;
...
let add (type a) t (node : a Node.t) =
  if verbose
  then Debug.ams _here_ "Recompute_heap.add" node <:sexp_of< _ Node.t >>;
  if debug && (Node.is_in_recompute_heap node || not (Node.needs_to_be_computed node))
  then failwiths "incorrect attempt to add node to recompute heap" node
         <:sexp_of< _ Node.t >>;
...
let remove (type a) t (node : a Node.t) =
  if verbose
  then Debug.ams _here_ "Recompute_heap.remove" (node, t) <:sexp_of< _ Node.t * t >>;
  if debug && (not (Node.is_in_recompute_heap node) || Node.needs_to_be_computed node)
  then failwiths "incorrect [remove] of node from recompute heap" node
         <:sexp_of< _ Node.t >>;
  unlink t node;
...
let increase_height (type a) t (node : a Node.t) =
  if verbose
  then Debug.ams _here_ "Recompute_heap.increase_height" node <:sexp_of< _ Node.t >>;
  if debug then begin
...
let min_height t =
  ...
  then t.height_lower_bound <- Array.length t.nodes_by_height
    ...
    while Uopt.is_none (Array.get nodes_by_height t.height_lower_bound) do
...
let remove_min t =
  ...
  let nodes_by_height = t.nodes_by_height in
  let node = ref (Array.get nodes_by_height t.height_lower_bound) in
  while Uopt.is_none !node do
    t.height_lower_bound <- t.height_lower_bound + 1;
    if debug && t.height_lower_bound >= Array.length t.nodes_by_height
    then failwiths "Recompute_heap.remove_min unexpectedly reached end of heap"
           t <:sexp_of< t >>;
    node := Array.get nodes_by_height t.height_lower_bound;
  done;
  let node = Uopt.unsafe_value !node in
  node.height_in_recompute_heap <- -1;
  t.length <- t.length - 1;
  let next = node.next_in_recompute_heap in
  Array.set t.nodes_by_height t.height_lower_bound next;
  if Uopt.is_some next
  then (Uopt.unsafe_value next).prev_in_recompute_heap <- Uopt.none;
