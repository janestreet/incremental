  ...
  Invariant.invariant [%here] t [%sexp_of: _ t] (fun () ->
    ...
    Fields.iter
      ~main:(check (fun (main : _ Node.t) ->
        ...
        | _ -> ()))
      ~test:ignore
      ~test_change:(check (fun (test_change : _ Node.t) ->
        match test_change.kind with
        | Invalid -> ()
        | If_test_change t' -> assert (same t t')
        | _ -> assert false))
      ~current_branch:(check (fun current_branch ->
        if Uopt.is_some current_branch then begin
          let current_branch = Uopt.value_exn current_branch in
          assert (phys_equal current_branch t.then_ || phys_equal current_branch t.else_)
        end))
      ~then_:ignore
      ~else_:ignore)
;;
