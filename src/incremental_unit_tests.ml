...
module Make () = struct
  ...
  module On_update_queue (Update : sig type 'a t [@@deriving compare, sexp_of] end) = struct
    let on_update_queue () =
      let r = ref [] in
      (fun e -> r := e :: !r),
      (fun expect ->
...
let%test_module _ = (module struct
  ...
  include (struct
    ...
    let%test _ = not (is_valid invalid)
    ...
    module Var = struct
      ...
      let%test_unit _ = (* [set] during stabilization gets the last value that was set *)
        let x = create_ [%here] 0 in
        let o =
    ...
    let%test _ = is_invalidated_on_bind_rhs (fun i -> bind (const i) (fun _ -> (const i)))
    ...
    let%test_unit _ = (* an invalid node created on the rhs of a valid bind, later invalidated *)
      let x = Var.create_ [%here] 13 in
      let r = ref None in
      let o1 =
    ...
    let%test_unit _ = (* invariants blow up here if we don't make sure that we first make the
    ...
    let%test_unit _ = (* plugging an invalid node in a bind can invalidate the bind (though
                   not always) *)
    ...
    let%test_unit _ = (* changing the rhs from a node to its ancestor, which causes problems if
    ...
    let%test_unit _ = (* changing branches from a node to its ancestor, which causes problems if
                   we leave the node with a broken invariant while adding the ancestor. *)
    ...
    let%test_unit _ = (* a frozen node remains valid, even if its original scope isn't *)
      let x = Var.create_ [%here] 13 in
      let r = ref None in
      let o1 =
        observe (watch x
    ...
    let%test_unit _ = (* a frozen node remains valid, even if the node it froze isn't *)
      let x = Var.create_ [%here] 13 in
    ...
    let%test_unit _ = (* a freeze that is stabilized and invalidated before it is frozen. *)
      let r = ref None in
      let x = Var.create_ [%here] 13 in
      let o = observe (bind (watch x) (fun i -> r := Some (const i); return ())) in
      stabilize_ [%here];
    ...
    let%test_unit _ = (* propagating the first argument of [depend_on] while the result of
    ...
    let%test _ = is_invalidated_on_bind_rhs (fun _ -> at (Time.add (Time.now ()) (sec 1.)))
    let%test _ = is_invalidated_on_bind_rhs (fun _ -> at (Time.add (Time.now ()) (sec (-1.))))
    ...
    let%test_unit _ = (* advancing the clock in the same stabilization cycle as creation *)
    ...
    let%test_unit _ = (* unobserved, [advance_clock] past [at] prior to stabilization. *)
      let x = Var.create_ [%here] 13 in
      let i =
        ok_exn (snapshot (Var.watch x) ~at:(Time.add (now ()) (sec 1.)) ~before:15)
      in
    ...
    let%test_unit _ = (* [snapshot] nodes increment [num_nodes_became_necessary] *)
      let i1 = State.(num_nodes_became_necessary t) in
      let c = const () in
      for _ = 1 to 5 do
    ...
    let%test_unit _ = (* Equivalence between [step_function] and reimplementation with [at] *)
    ...
    let%test_unit _ = (* alarms get cleaned up for invalidated time-based incrementals *)
      List.iter
    ...
    module Observer = struct
      ...
      let%test_unit _ = (* calling [value] after [disallow_future_use] returns error. *)
        let x = Var.create_ [%here] 0 in
        let o = observe (watch x) in
        stabilize_ [%here];
        disallow_future_use o;
      ...
      let%test_unit _ = (* [disallow_future_use] and finalize in the same stabilization. *)
        let x = Var.create_ [%here] 1 in
        let o = observe (Var.watch x) in
        stabilize_ [%here];
        disallow_future_use o;
      ...
      let%test_unit _ = (* after user resurrection of an observer, it is still disallowed *)
        let x = Var.create_ [%here] 13 in
        let o = observe (Var.watch x) in
        stabilize_ [%here];
        Gc.keep_alive o;
      ...
      let%test_unit _ = (* lots of observers on the same node isn't quadratic. *)
        ...
        if not debug then begin
          ...
          if verbose then Debug.ams [%here] "consumed" consumed [%sexp_of: Time.Span.t];
      ...
      let%test_unit _ = (* [on_update_exn] of an invalid node, not during a stabilization *)
      ...
      let%test_unit _ = (* stabilizing with an on-update handler of a node that is invalidated *)
      ...
      let%test_unit _ = (* disallowing other on-update handlers in an on-update handler *)
        let x = Var.create_ [%here] 13 in
        let o = observe (watch x) in
        for _ = 1 to 2 do
      ...
      let%test_unit _ = (* disallowing other observers of the same node an on-update handler *)
        let x = Var.create_ [%here] 13 in
      ...
      let%test_unit _ = (* disallowing observers of other nodes in an on-update handler *)
      ...
      let%test_unit _ = (* adding an on-update-handler to an invalid node in an on-update
                     handler. *)
      ...
      let%test_unit _ = (* on-update handler set up during stabilization fires after the
                     stabilization *)
      ...
      let%test_unit _ = (* creating an observer and adding on_update handler during
                     stabilization *)
      ...
      let%test_unit _ = (* creating an observer and disallowing use during stabilization *)
        let x = Var.create 13 in
        let r = ref None in
        let o1 =
          observe (Var.watch x
      ...
      let%test_unit _ = (* creating an observer and finalizing it during stabilization *)
        let x = Var.create 13 in
        let o =
          observe (Var.watch x
                   >>| fun _ ->
    ...
    let%test_unit _ = (* on-change handlers of a node that changes but is not necessary
    ...
    let%test_unit _ = test_memoize_fun (fun hashable f -> memoize_fun_by_key hashable Fn.id f)
    ...
    let%test_unit _ = (* Deleting a parent from a child in such a way that it is replaced by a
...
let%test_unit _ = (* trying to make a node necessary without its scope being necessary *)
  let module I = Make () in
  let open I in
  let r = ref None in
  let x = Var.create 13 in
