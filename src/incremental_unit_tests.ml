    ...
    (module struct
      ...
      include (struct
        ...
        let%test_unit _ =
          ...
          assert (Time.equal (value w) before_advance); (* we didn't yet stabilize *)
          stabilize_ [%here];
        ...
        let%test _ =
          is_invalidated_on_bind_rhs (fun _ -> at (Time.add (Time.now ()) (sec 1.)))
        ;;
        ...
        let%test_unit _ = (* [at] in the past *)
          assert (is_error
                    (snapshot (const 14) ~at:(Time.sub (now ()) (sec 1.)) ~before:13))
        ;;
        ...
        let%test_unit _ = (* [at] in the future *)
          let o =
            observe
              (ok_exn
                 (snapshot (const 14) ~at:(Time.add (now ()) (sec 1.)) ~before:13))
          in
          stabilize_ [%here];
          assert (value o = 13);
          stabilize_ [%here];
        ...
        let%test_unit _ = (* [at] in the future, unobserved *)
          ...
          let i =
            ok_exn
              (snapshot (Var.watch x) ~at:(Time.add (now ()) (sec 1.)) ~before:15)
          in
          stabilize_ [%here];
          Var.set x 17;
          advance_clock_by (sec 2.);
        ...
        let%test_unit _ = (* [advance_clock] past [at] prior to stabilization. *)
          let o =
            observe
              (ok_exn
                 (snapshot (const 15) ~at:(Time.add (now ()) (sec 1.)) ~before:13))
          in
          advance_clock_by (sec 2.);
          stabilize_ [%here];
          assert (value o = 15)
        ...
        let%test_unit _ = (* invalidated *)
          let t =
            ok_exn (snapshot invalid ~at:(Time.add (now ()) (sec 1.)) ~before:13)
          in
          let o = observe t in
          stabilize_ [%here];
          assert (value o = 13);
        ...
        let%test_unit _ =
          ...
          for _ = 1 to 5 do
            ignore
              (ok_exn (snapshot c ~at:(Time.add (now ()) (sec 1.)) ~before:()) : _ t);
          done;
          advance_clock_by (sec 2.);
          let i2 = State.(num_nodes_became_necessary t) in
          [%test_result: int] i2
        ...
        module Observer = struct
          ...
          let%test_unit _ = (* lots of observers on the same node isn't quadratic. *)
            ...
            if not debug then begin
              ...
              if verbose
              then Debug.ams [%here] "consumed" consumed [%sexp_of: Time.Span.t];
      ...
      let%test_unit _ = (* snapshot cycle in the future *)
        ...
        (* [advance_clock] should raise because the snapshot's [value_at] depends on
           the snapshot itself. *)
        assert (does_raise (fun () ->
          advance_clock ~to_:(Time.add (now ()) (sec 2.))));
        Gc.keep_alive (o1, o2)
      ;;
